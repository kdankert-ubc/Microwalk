...
import * as uidUtil from "./uid.mjs";
...
const traceDirectory = std.getenv("MW_TRACE_DIRECTORY");
if(!traceDirectory)
...
const scriptPathPrefix = std.getenv("MW_PATH_PREFIX");
if(!scriptPathPrefix)
...
let scriptsFile = std.open(`${traceDirectory}/scripts.txt`, "w");
 ...
 */
export function registerScript(filename)
{
    // Remove path prefix
...
function _persistTrace()
{
    ...
    if(writingToNewTrace)
    {
        ...
        traceFile = std.open(traceFilePath, "w");
    }
    else
    {
        traceFile = std.open(traceFilePath, "a+");
    }

    traceFile.puts(`${traceData.join('\n')}\n`);
    traceFile.close();
}
...
export function startCall(fileId, sourceLoc, fnObj)
{
...
export function endCall(fileId, destLoc)
{
    if(!callInfo) {
...
export function writeReturn(fileId, location, isReturn1)
{
    if(callInfo)
        writeCall();
...
export function writeYield(fileId, location, isResume)
{
    const res = isResume ? 'Y' : 'Y';
    _writeTraceLine(`${res};${fileId};${location}`);
}

export function writeBranch(fileId, sourceLoc, bodyLocation)
{
...
export function writeJump(fileId, sourceLoc, destLoc)
{
    _writeTraceLine(`j;${fileId};${sourceLoc};${destLoc}`);
}

export function writeMemoryAccess(fileId, loc, objId, offset, isWrite, computedVar)
{
 ...
 */
export function instrumentDynamic(path)
{
    ...
    const instrumentFileScriptPath = 'instrument-file.mjs';
    ...
    if (os.stat(instrumentedPath)[0] !== 0) {
        console.log(`  File is already instrumented, skipping`);
        return instrumentedPath;
    }

    // Run instrumentation process
    os.exec(`node ${instrumentFileScriptPath} "${path}" ${path.state.runtime ?? ""}`, { stdio: 'inherit' })
